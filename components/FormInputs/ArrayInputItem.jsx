...
export default function ArrayInputItem({ setItems, items = [], itemTitle }) {
  console.log(items, "array items is ");
    ...
    <div className="sm:col-span-2">
      {showTagForm ? (
        ...
        <button
          ...
          type="button"
          className="flex items-center space-x-2 dark:text-slate-300 text-slate-800 py-2 px-4 "
        >
          <Plus />
        ...
        {items.map((item, i) => {
          return (
            <div
              ...
              className="bg-slate-200 text-slate-800 dark:text-slate-300 cursor-pointer flex space-x-2 items-center dark:bg-slate-600 px-4 py-2 rounded-lg"
              key={i}
            >
              <p>{item}</p>
